...
//4)Listedeki tekrarlı elemanları silip tek sayı olanların küplerini yazdıran metod oluştur.
	...
	// 5)Listedeki tekrarlı elemanları silip çift sayı olanların karelerinin
	// toplamını yazdıran metod oluştur
...
//6)Listedeki tekrarlı elemanları silip çift sayı olanların küplerinin çarpımını yazdıran metod oluştur
	private static void ciftSayiKup(List<Integer> l) {
		int kup = l.stream().distinct().filter(t->t%2==0).map(t->t*t*t).reduce(1, (a,b)-> a*b);
		System.out.println(kup);
	}
	
// Listedeki elemanlari duz ve ters olarak siralayin	

	private static void tersDuzSirala(List<Integer> l) {
